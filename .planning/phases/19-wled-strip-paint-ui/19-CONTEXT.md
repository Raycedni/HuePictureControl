...
### Orientation (per-region sub-sample axis)

> **Planning-time narrowing (2026-05-14, decided during /gsd-plan-phase 19):** Orientation is **per-region**, not per-assignment. All WLED channels assigned to the same region in the same entertainment config share one orientation. This preserves D-22's `{region_id: ndarray}` coordinator-to-sink contract literally (one gradient per region, no nested dict). The deferred "user-picks-axis at the region level" item is **promoted** to Phase 19 scope; the per-assignment variant is dropped.

- **D-16:** New column **`wled_light_assignments.orientation TEXT NOT NULL DEFAULT 'auto'`** added via a schema migration in Phase 19. Storage is still on `wled_light_assignments` (avoids a new table), but the **API and UI treat orientation as region-scoped**: changing orientation for region R in config C writes the same value to every row matching `(region_id=R, entertainment_config_id=C)`. The same channel assigned to two different regions can still have two different orientations (orientation keys on region, not channel). Phase 17's auto-seeded channel assignments backfill to `'auto'` via the `DEFAULT` clause — no explicit data migration step needed.
...
- **D-19:** The orientation control surfaces in a **region properties panel on the canvas side**, not in the channel sidebar. Selecting a region (existing `useRegionStore.selectedId` pattern) opens a properties surface that shows **one 5-option segmented control for the entire region** (auto / ← / → / ↑ / ↓), plus a read-only list of the WLED channels assigned to that region (chip + name) so the user knows which channels the orientation applies to. Exact placement of this surface (popover anchored to the selected region, inline expansion under `LightPanel.Assignments`, dedicated side panel) is Claude's discretion in planning — it must coexist with existing region selection without breaking the canvas drag-drop drop target.
...
- **D-21:** New endpoints under the existing `routers/wled.py`:
  ...
  - `PUT /api/wled/assignments` — upsert `(region_id, wled_channel_id, entertainment_config_id)`. New assignments inherit the region's current orientation (or `'auto'` if none). Mirrors the per-config scoping from Phase 17 D-08.
  - `PATCH /api/wled/regions/{region_id}/orientation?config={entertainment_config_id}` — **region-scoped** orientation update. Body: `{orientation: <enum>}`. Writes the same orientation to every `wled_light_assignments` row matching `(region_id, entertainment_config_id)`.
  - `DELETE /api/wled/assignments/...` — remove an assignment (driven by the canvas-side properties panel or LightPanel "Clear" action).
- **D-22:** Coordinator gradient contract from Phase 17 D-05 (`{region_id: gradient_array}`) is **unchanged literally**. Per-region orientation (D-16) guarantees one orientation per region, so `sub_sample_gradient` is called once per region with the region's resolved orientation. `N_region = max(channel_width for every assigned wled_channel)` continues to apply; the orientation flag affects axis + direction, not sample count.
